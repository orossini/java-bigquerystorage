...
import com.example.bigquerystorage.TestProto2.Test2;
...
public class WriteProtoToDefaultStream {
    ...
    private static ProtoRows buildRecord() {

        Test2.Builder test2 = Test2.newBuilder();
        test2.setId(0);
        test2.setNameSt("test 2 - defaultStream-java");
        test2.setAge(1);
        test2.setSourceRecordId(2);
        test2.setServiceFileId("servFileId0123");
        test2.setEndTransaction(true);

        ProtoRows.Builder protorowbuilder = ProtoRows.newBuilder();
        ProtoRows record = protorowbuilder.addSerializedRows(test2.build().toByteString()).build();
        return record;
    }
    ...
    public static void writeToDefaultStream(String projectId, String datasetName, String tableName)
        ...
        // Write two batches of fake data to the stream, each with 1 record. Data
        // may be batched up to the maximum request size:
        // https://cloud.google.com/bigquery/quotas#write-api-limits
        for (int i = 0; i < 2; i++) {
            ProtoRows record = buildRecord();
            writer.append(new AppendContext(record, 0));
        }

        // Final cleanup for the stream during worker teardown.
        writer.cleanup();
        // Adjust the number of expectedRowCount depending on the test to run. Here we
        // have 2 batchres of 1 record each. (the destination table is expected tobe
        // empty at verifyExpectedRowCount)
        verifyExpectedRowCount(parentTable, 2);
    ...
    private static class DataWriter {
        ...
        public void initialize(TableName parentTable)
            ...
            Descriptor descriptor = Test2.getDescriptor();
            ProtoSchema protoSchema = ProtoSchemaConverter.convert(descriptor);

            streamWriter = StreamWriter
                    .newBuilder(parentTable.toString() + "/streams/_default", BigQueryWriteClient.create())
                    .setExecutorProvider(
                    ...
                    // If value is missing in json and there is a default value configured on
                    // bigquery
